...
export type PromptType = 'yesNo' | 'smallText' | 'bigText' | 'plainText';

export const promptType: Record<PromptType, PromptType> = {
  yesNo: 'yesNo',
  smallText: 'smallText',
  bigText: 'bigText',
  plainText: 'plainText',
...
export interface Prompt {
  text: string;
  type: PromptType;
  /** Conditional follow-up shown beneath the prompt (e.g. "If so, how?"). */
  sub?: Prompt;
}
...
export const fullText: InventoryPage[] = [
  {
    title: 'What Worked for Us: NA Daily Journal',
    prompts: [
      {
        text: 'The purpose of this daily inventory is to keep track of who we are and what we are doing today.',
        type: promptType.plainText,
      },
      {
        text: ' Try to answer every question, every day. Keep your answers as simple and as honest as you can. If you miss a day, do not make it up later.',
        type: promptType.plainText,
      },
      {
        text: 'Limit your answers and concerns to the past twenty-four hours and complete the questions at the end of the day.',
        type: promptType.plainText,
      },
      {
        text: 'If you wish to share some of this journal, we suggest that you do so verbally.',
        type: promptType.plainText,
      },
      {
        text: 'Remember that a daily inventory is not a test; there are no right or wrong answers. Do not get upset if you could have done better or if you fail to live up to your expectations of yourself. Remember, this is a program of growth and recovery and if you are clean today, you are doing something right.',
        type: promptType.plainText,
      },
    ],
  },
...
/** Number of the last inventory page (0-indexed). */
export const LAST_PAGE = fullText.length - 1;
...
/**
 * Ordered list of every answerable prompt key (questions + sub-prompts + Notes),
 * skipping the intro's plain-text lines. Order matches the legacy `outputForm`
 * object, which the PDF export relies on.
 */
export const answerKeys: string[] = fullText
  .flatMap((page) => page.prompts)
  .flatMap((prompt) =>
    prompt.type === promptType.plainText
      ? []
      : prompt.sub
        ? [prompt.text, prompt.sub.text]
        : [prompt.text],
  );

/** A fresh, empty answers object keyed by every prompt. */
export function createEmptyAnswers(): Answers {
  return Object.fromEntries(answerKeys.map((key) => [key, ''])) as Answers;
}
